...
import { LogIn, LogOut, Plus, Users, X, Bell, Check, XCircle, Menu } from "lucide-react";
import axios from "axios";
import { useState, useEffect, useRef } from "react";
import { Input } from "@/components/ui/input";
import { useNotifications } from "@/app/hooks/useNotifications";
                ...
                <SheetContent side="right" className="w-72 sm:w-80 p-0 flex flex-col">
                        ...
                        <div className="space-y-2 max-h-48 overflow-y-auto">
                          {requests.map((request) => (
                            <div key={request.id} className="p-3 bg-accent/5 rounded-lg border border-accent/20 space-y-2">
                              <div>
                                <p className="text-sm font-medium text-foreground">User {request.userId}</p>
                  ...
                  <div className="border-t border-accent/20 p-6">
                    <Button
                      ...
                      className="w-full gap-2 rounded-lg hover:bg-destructive/10 hover:text-destructive text-foreground transition-colors text-sm justify-start"
                    >
                      <LogOut className="h-4 w-4" />
                      Logout
                    </Button>
